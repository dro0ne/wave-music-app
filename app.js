const WAVE_BUILD='multi-source-preload-20260923';
...
const PLAYER_STATES={STOPPED:'STOPPED',LOADING:'LOADING',PLAYING:'PLAYING',PAUSED:'PAUSED',ERROR:'ERROR'};
let playerState=PLAYER_STATES.STOPPED,playbackBuffer={current:null,next:null,backupNext:null},preloadMedia=null,preloadReady=false,preloadRequestId=0,bufferFillPromise=null,nextStartedAt=0;
...
async function validateCandidate(selected,rank){
  ...
  try{let resolved=await sourceManager.resolveTrack(track);if(cached)return resolved;let result=await validateStreamUrl(resolved.stream);console.log('STREAM_VALIDATE_OK',{trackId:id,status:result.status,contentType:result.contentType,source:resolved.activeSource,validation:result.validation});verifiedPlayableTrackIds.set(id,Date.now()+PLAYABLE_CACHE_TTL);return resolved}
  catch(error){console.warn('STREAM_VALIDATE_FAILED',{trackId:id,title:track.title,rank,status:error?.httpResult?.status||0,error:error.message});failedTrackIds.add(id);return null}
}
...
async function choosePlayableTrack(reservedIds=new Set()){
  let candidates=catalog.filter(t=>!failedTrackIds.has(WaveRecommendation.idOf(t))&&!reservedIds.has(WaveRecommendation.idOf(t))&&(sourceManager.sources.has(t.source)||!!t.stream||t.source==='Local Demo'));
...
function setPlayerState(nextState){if(playerState===nextState)return;playerState=nextState;let button=$('#play'),icon=button.querySelector('.control-icon');button.dataset.state=nextState;icon.textContent=nextState===PLAYER_STATES.PLAYING?'Ⅱ':nextState===PLAYER_STATES.ERROR?'!':'▶';button.setAttribute('aria-label',nextState===PLAYER_STATES.PLAYING?'Пауза':nextState===PLAYER_STATES.ERROR?'Повторить воспроизведение':nextState===PLAYER_STATES.LOADING?'Загрузка':'Воспроизвести');document.body.classList.toggle('wave-playing',nextState===PLAYER_STATES.PLAYING);console.log('PLAYER_STATE',nextState)}
...
function invalidateNextBuffer(reason='manual'){preloadRequestId++;playbackBuffer.next=null;playbackBuffer.backupNext=null;cancelPreloadAudio();console.log('PRELOAD_INVALIDATED',{reason,requestId:preloadRequestId});if(current()&&reason!=='start')void fillPlaybackBuffer()}
...
async function play(){if(!current())return start();if(playbackStoppedOnError){playbackStoppedOnError=false;return next('error',true)}tabChannel?.postMessage('claim-audio');playing=true;setPlayerState(PLAYER_STATES.LOADING);if('mediaSession'in navigator)navigator.mediaSession.playbackState='playing';sound(current());clearInterval(timer);timer=setInterval(()=>{elapsed=media?Math.floor(media.currentTime):elapsed+1;if(elapsed>=current().duration)return next('ended');$('#time').textContent=fmt(elapsed);$('#progress').value=elapsed/current().duration*100;paintRange($('#progress'))},1000);void fillPlaybackBuffer()}
function pause(){playing=false;setPlayerState(current()?PLAYER_STATES.PAUSED:PLAYER_STATES.STOPPED);if('mediaSession'in navigator)navigator.mediaSession.playbackState='paused';clearInterval(timer);if(media)media.pause();else stopSound()}
...
async function next(event='next',alreadyRecorded=false){if(!current())return start();if(selectionInProgress)return;selectionInProgress=true;nextStartedAt=performance.now();let resume=playing;if(!alreadyRecorded)recordPlayback(event);let chosen=playbackBuffer.next;if(chosen){console.log('PRELOAD_SWAP',{trackId:chosen.id,title:chosen.title,ready:preloadReady});playbackBuffer.next=playbackBuffer.backupNext;playbackBuffer.backupNext=null}else{setPlayerState(PLAYER_STATES.LOADING);$('#title').textContent='Подбираем доступный трек…';$('#artist').textContent='Проверяем источники аудио';try{chosen=await choosePlayableTrack()}catch{} }selectionInProgress=false;if(!chosen)return toast('Не удалось подобрать следующий трек с доступным аудио');if(media&&preloadMedia!==media)media.pause();if(event!=='error')playbackRetryCount=0;playbackStoppedOnError=false;queue=queue.slice(0,index+1);queue.push(chosen);index=queue.length-1;load();if(resume||['next','ended','skip','dislike','error'].includes(event))void play();void fillPlaybackBuffer()}
function prev(){pause();index=(index-1+queue.length)%queue.length;load();play()}
function handlePlayRejection(error){if(error?.name==='NotAllowedError')return showPlayRequired();if(error?.name==='AbortError')return;setTimeout(()=>{if(!media?.error){playing=false;setPlayerState(PLAYER_STATES.ERROR);clearInterval(timer);console.warn('PLAYBACK_START_REJECTED',{name:error?.name,message:error?.message});$('#artist').textContent='Не удалось запустить аудио. Нажми ▶, чтобы повторить.'}},0)}
async function trySameTrackFallback(track){let alternative=(track.alternatives||[]).find(x=>x.source!==track.activeSource);if(!alternative)return false;try{let resolved=await sourceManager.resolveTrack({...alternative,alternatives:[]}),replacement={...track,stream:resolved.stream,streamUrl:resolved.streamUrl,activeSource:resolved.activeSource,alternatives:(track.alternatives||[]).filter(x=>x.source!==resolved.activeSource)};queue[index]=replacement;playbackBuffer.current=replacement;console.log('SOURCE_FALLBACK',{trackId:track.id,from:track.activeSource||track.source,to:replacement.activeSource});sound(replacement);return true}catch(error){console.warn('SOURCE_FALLBACK_FAILED',{trackId:track.id,source:alternative.source,error:error.message});return false}}
function sound(t){
  if(t.stream){
    let prepared=preloadMedia?.dataset.trackId===String(t.id);if(prepared){if(media&&media!==preloadMedia){media.pause();media.onended=null;media.onerror=null}media=preloadMedia;preloadMedia=null;preloadReady=false}else if(media?.dataset.trackId!==String(t.id)){let unlocked=media?.dataset.unlock==='true';if(!unlocked){stopSound();media=new Audio()}else{media.pause();media.removeAttribute('src')}}delete media.dataset.unlock;media.dataset.trackId=String(t.id);media.preload='auto';media.volume=+$('#volume').value/100;media.onended=()=>next('ended');
    ...
    media.onplay=()=>setPlayerState(PLAYER_STATES.LOADING);media.onwaiting=media.onstalled=()=>setPlayerState(PLAYER_STATES.LOADING);media.onpause=()=>{if(!media.ended&&playerState!==PLAYER_STATES.LOADING)setPlayerState(PLAYER_STATES.PAUSED)};media.oncanplay=()=>{clearTimeout(loadTimer);$('#artist').textContent=originalMeta};media.onplaying=()=>{setPlayerState(PLAYER_STATES.PLAYING);playbackRetryCount=0;playbackStoppedOnError=false;if(nextStartedAt){console.log('NEXT_LATENCY_MS',Math.round(performance.now()-nextStartedAt));nextStartedAt=0}};
    media.onerror=()=>{clearTimeout(loadTimer);let failed=current(),error=media?.error,wasPlaying=playing;console.warn('PLAYBACK_FAILED',{trackId:failed?.id,title:failed?.title,artist:failed?.artist,stream:failed?.stream,mediaErrorCode:error?.code||0,mediaErrorMessage:error?.message||'',networkState:media?.networkState,readyState:media?.readyState});setPlayerState(PLAYER_STATES.LOADING);void trySameTrackFallback(failed).then(recovered=>{if(recovered)return;failedTrackIds.add(WaveRecommendation.idOf(failed));playing=false;setPlayerState(PLAYER_STATES.ERROR);clearInterval(timer);if(wasPlaying&&playbackRetryCount<MAX_PLAYBACK_RETRIES){playbackRetryCount++;console.log('PLAYBACK_RETRY',{attempt:playbackRetryCount,maxAttempts:MAX_PLAYBACK_RETRIES});toast(`Поток недоступен — попытка ${playbackRetryCount} из ${MAX_PLAYBACK_RETRIES}`);setTimeout(()=>next('error',true),500)}else{playbackStoppedOnError=true;$('#artist').textContent='Не удалось получить аудио от источника. Нажми ▶, чтобы попробовать другой трек.';toast('Источник аудио недоступен — автоматические попытки остановлены')}})};
    if(!prepared&&media.src!==t.stream)media.src=t.stream;media.play().catch(error=>{clearTimeout(loadTimer);handlePlayRejection(error)});return
  }
